...
void stack_op_divmod (stack_t* stk) {
  ...
  stack_push(stk, number_divmod(b, a, &mod));
  stack_push(stk, mod);
...
void    stack_op_prc (stack_t* stk) {
  ...
  printf("%LG", floorl( stack_pop(stk)->value ) );
}
/*
  effect: ( x -- x )
