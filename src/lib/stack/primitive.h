...
void stack_set (stack_t* stk, const ssize_t idx, const number_t* const val) {
  pfn();
...
*/
void stack_push (stack_t* stk, const number_t* const val) {
  pfn();
...
number_t* stack_pop (stack_t* stk) {
  ...
  stack_set(stk, stk->ptr, number_new(0));
  stack_decr(stk);
  return out;
}
...
*/
size_t atoi_strlen (const number_t* const val) {
  pfn();

  char*  buf = (typeof(buf)) safemalloc(100);
  size_t len = signed2un(snprintf(buf, 100, "%LG", val->value));
...
number_t** str_to_number_array (
  ...
  for (size_t i = 0; i < len; i++) {
    out[i] = number_new(strtold(split[i], NULL));
  }
